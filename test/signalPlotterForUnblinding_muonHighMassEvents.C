...
void MakeHistos(TChain * chain, Selector *myEvent, std::vector<TH1F*> *hs, Selector::tag_t channel){
	...
	for(int ev = 0; ev<nEntries; ++ev){
		...
		if (myEvent->WR_mass > 3500.) {

			// cout<<"event "<<myEvent->event<<", run "<<myEvent->run<<", lumi "<<myEvent->lumi;
			// cout<<": mWR="<<myEvent->WR_mass<<", pt_mu1="<<myEvent->lead_lepton_pt<<", eta_mu1="<<myEvent->lead_lepton_eta<<", phi_mu1="<<myEvent->lead_lepton_phi;
			// cout<<", pt_mu2="<<myEvent->sublead_lepton_pt<<", eta_mu2="<<myEvent->sublead_lepton_eta<<", phi_mu2="<<myEvent->sublead_lepton_phi;
			// cout<<", pt_j1="<<myEvent->lead_jet_pt<<", eta_j1="<<myEvent->lead_jet_eta<<", phi_j1="<<myEvent->lead_jet_phi;
			// cout<<", pt_j2="<<myEvent->sublead_jet_pt<<", eta_j2="<<myEvent->sublead_jet_eta<<", phi_j2="<<myEvent->sublead_jet_phi<<endl;

			cout<<"run:lumi:event = "<<myEvent->run<<":"<<myEvent->lumi<<":"<<myEvent->event;
			cout<<", mWR = "<<myEvent->WR_mass;
			cout<<", mu1 = ("<<myEvent->lead_lepton_pt<<","<<myEvent->lead_lepton_eta<<","<<myEvent->lead_lepton_phi<<")";
			cout<<", mu2 = ("<<myEvent->sublead_lepton_pt<<","<<myEvent->sublead_lepton_eta<<","<<myEvent->sublead_lepton_phi<<")";
			cout<<", j1 = ("<<myEvent->lead_jet_pt<<","<<myEvent->lead_jet_eta<<","<<myEvent->lead_jet_phi<<")";
			cout<<", j2 = ("<<myEvent->sublead_jet_pt<<","<<myEvent->sublead_jet_eta<<","<<myEvent->sublead_jet_phi<<")";
			cout<<", mll = "<<myEvent->dilepton_mass<<endl;
...
void drawPlots(TH1F* hs_DY,TH1F* hs_ttbar,TH1F* hs_others,TH1F* hs_data,TH1F* hs_signal_1,TH1F* hs_signal_2,TH1F* hs_signal_3, TString xtitle, TString ytitle, TString fname, Selector::tag_t channel){
	...
	// for adding overflow
	if(xtitle == "Mlljj (GeV)"){
		cout << "last bin: data=" << hs_data->GetBinContent(hs_data->GetNbinsX()) << ", DY=" << hs_DY->GetBinContent(hs_DY->GetNbinsX()) << ", ttbar=" << hs_ttbar->GetBinContent(hs_ttbar->GetNbinsX()) << ", Other=" << hs_others->GetBinContent(hs_others->GetNbinsX()) << endl;
		cout << "overflow: data=" << hs_data->GetBinContent(hs_data->GetNbinsX()+1) << ", DY=" << hs_DY->GetBinContent(hs_DY->GetNbinsX()+1) << ", ttbar=" << hs_ttbar->GetBinContent(hs_ttbar->GetNbinsX()+1) << ", Other=" << hs_others->GetBinContent(hs_others->GetNbinsX()+1) << endl;
		// cout << "last bin with overflow: data=" << hs_data->GetBinContent(hs_data->GetNbinsX())+hs_data->GetBinContent(hs_data->GetNbinsX()+1) << ", DY=" << hs_DY->GetBinContent(hs_DY->GetNbinsX())+hs_DY->GetBinContent(hs_DY->GetNbinsX()+1) << ", ttbar=" << hs_ttbar->GetBinContent(hs_ttbar->GetNbinsX())+hs_ttbar->GetBinContent(hs_ttbar->GetNbinsX()+1) << ", Other=" << hs_others->GetBinContent(hs_others->GetNbinsX())+hs_others->GetBinContent(hs_others->GetNbinsX()+1) << endl;
		...
		cout << "last bin with overflow: data=" << hs_data->GetBinContent(hs_data->GetNbinsX()) << ", DY=" << hs_DY->GetBinContent(hs_DY->GetNbinsX()) << ", ttbar=" << hs_ttbar->GetBinContent(hs_ttbar->GetNbinsX()) << ", Other=" << hs_others->GetBinContent(hs_others->GetNbinsX()) << endl;
	}
	...
	//for overflow
	// if(xtitle == "Mlljj (GeV)"){
	// 	cout<<"Mlljj bk OF="<<((TH1*)th->GetStack()->Last())->GetBinContent(hs_data->GetNbinsX())<<" "<<((TH1*)th->GetStack()->Last())->GetBinContent(hs_data->GetNbinsX()+1)<<endl;
	// 	cout<<"Mlljj data OF="<<hs_data->GetBinContent(hs_data->GetNbinsX())<<" "<<hs_data->GetBinContent(hs_data->GetNbinsX()+1)<<endl;
	// }
	...
	errors->SetBinContent(errors->GetNbinsX(), 0.0101); //aggiunto banda in ultimo bin che ha zero events
	...
	if(xtitle == "Mlljj (GeV)"){
		...
		for(int i = 0;i<hs_data->GetNbinsX()+1;i++){	
			...
			cout << ", errorSum = " << errors->GetBinError(i) << endl; 
		}
	...
	TString fn = "";
	TString outputdir = "";
	outputdir = "withOverflow/";

	// if(channel == Selector::EMu)
	//   fn = "~/www/WR/plots/miniTree/Selected/Flavor/"+fname;
		//fn = "plots/Flavor/"+fname;
	if(channel == Selector::EE)
		fn = "/afs/cern.ch/user/g/gnegro/www/cmsWR/unblinding/EESignalRegion/"+outputdir+fname;
	//fn = "plots/EELowDilepton/"+fname;
	if(channel == Selector::MuMu)
		// fn = "/afs/cern.ch/user/g/gnegro/www/cmsWR/unblinding/MuMuSignalRegion/"+outputdir+fname;
		fn = "/afs/cern.ch/user/g/gnegro/www/WR16/"+fname;
