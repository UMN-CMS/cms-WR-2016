...
// void drawPlots(TH1F* hs_DY,TH1F* hs_ttbar,TH1F* hs_others,TH1F* hs_signal_1,TH1F* hs_signal_2,TH1F* hs_signal_3,TString xtitle, TString ytitle, TString fname, Selector::tag_t channel);
...
void Plotter(Selector::tag_t channel){
	...
	TString inputDirSignal = "/eos/cms/store/group/phys_exotica/leptonsPlusJets/WR/selectedTrees_WRv07/selectedTrees_WRv07_newXsections/";
	...
	case Selector::EE:
		...
		chain_signal_1->Add(inputDirSignal+"selected_tree_WRtoEEJJ_2000_1000_signal_eeEE.root");
		chain_signal_2->Add(inputDirSignal+"selected_tree_WRtoEEJJ_3000_1500_signal_eeEE.root");
		chain_signal_3->Add(inputDirSignal+"selected_tree_WRtoEEJJ_4000_2000_signal_eeEE.root");
		break;
	case Selector::MuMu:
		...
		chain_signal_1->Add(inputDirSignal+"selected_tree_WRtoMuMuJJ_2000_1000_signal_mumuMuMu.root");
		chain_signal_2->Add(inputDirSignal+"selected_tree_WRtoMuMuJJ_3000_1500_signal_mumuMuMu.root");
		chain_signal_3->Add(inputDirSignal+"selected_tree_WRtoMuMuJJ_4000_2000_signal_mumuMuMu.root");
		break;
	default:
	...
	"dN/dm_{l_{1}jj} (GeV^{-1})","dM/dm_{l_{2}jj} (GeV^{-1})", "dN/dn_{jets}"};
	...
	for(unsigned int i = 0; i < nPlots; i++){
		...
		// drawPlots(hs_DY[i],hs_ttbar[i],hs_others[i],hs_signal_1[i],hs_signal_2[i],hs_signal_3[i],xtitles[i],ytitles[i],fnames[i], channel);
	}
...
// void drawPlots(TH1F* hs_DY,TH1F* hs_ttbar,TH1F* hs_others,TH1F* hs_signal_1,TH1F* hs_signal_2,TH1F* hs_signal_3, TString xtitle, TString ytitle, TString fname, Selector::tag_t channel){
	...
	TLegend *leg = new TLegend( 0.46, 0.6, 0.9, 0.9 ) ; 
	...
	// hs_signal_1->GetYaxis()->SetTitle(ytitle);
	// hs_signal_1->GetYaxis()->SetTitleSize(0.05);
	// hs_signal_1->GetYaxis()->SetTitleOffset(0.9); 
	// hs_signal_1->GetYaxis()->SetLabelSize(0.04);	
	// hs_signal_1->GetXaxis()->SetLabelSize(0);	
	...
	if(channel == Selector::EE)
		fn = outputdir+"EESignalRegion_blinded/"+dir+fname;
	if(channel == Selector::MuMu)
		fn = outputdir+"MuMuSignalRegion_blinded/"+dir+fname;
